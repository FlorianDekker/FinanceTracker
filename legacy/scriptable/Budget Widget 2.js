...
// ===============================
// Budget Widget 2 (Hybrid, NO header)
// Categories: afspreken_vrienden, kleding, vakantie, cadeaus_overig
//
// Added pace indicator (same as Widget 1):
...
const rows = ORDER.map(({ key, icon }) => {
  ...
  // if spentBefore > budget*(months passed), you're behind by that amount.
  const paceOverspendBefore = spentBefore - budget * (CUR_MONTH - 1)
  const paceBuffer = Math.max(0, paceOverspendBefore)

  // Clamp buffer to remaining (no negative / no nonsense)
